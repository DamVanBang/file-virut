...
const state = {
  spacing: _.range(0, 64).map(n => '&nbsp;').join(''),
};
...
class File extends _.EventEmitter {
  ...
  constructor (filename, doc) { super();
    _.define.box(this, 'meta', {
      ...
      file: {
        ...
        name: _.strcase.camel(_.path.basename(filename, _.path.extname(filename))),
      },
    });
  }
  ...
  getDocs () {
    ...
    item.docs = docs;
    item.info = info;
    return docs.map(doc => ({ item, info, ...doc }));
  }
